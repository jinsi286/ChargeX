...
if(isset($_POST["login"]))
{
    ...
    $qry = "select * from stationreg where email = '$email' && password = '$password'";
    ...
    if($count == 1)
    {
        ...
        $_SESSION["name"] = $row[1];
        $_SESSION["s_id"] = $row[0];
        echo $qry = "select approved from admin where s_id = $row[0]";
        $res = mysqli_query($con,$qry);
        $row = mysqli_fetch_row($res);
        if($row[0])
        {
                header("location:stationDashboard.php");
            }
            else
            {
                            ...
                            <form class="form-horizontal" id="popup-validation" name="frm" action="#" method="post">
                                <div class="form-group">
                                        ...
                                        <div style="text-align:center" class="form-actions no-margin-bottom">
                                            ...
                                            <a href="stationReg.php">New User?</a>
                                        </div>
                                    </form>
                        </div>
